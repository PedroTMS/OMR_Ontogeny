...
# --- CONFIGURATION FLAGS ---
MEGABOUTS_FLAG = True  # Set to True to use the megabouts library
...
def analyze_behavior(tail_angle_array, fs=700, use_megabouts=False):
    ...
    Args:
        ...
        fs (int): Sampling frequency.
        use_megabouts (bool): Flag to select detection algorithm.
...
def process_recording(row):
    ...
    # --- 4. Merge Data ---
    cam_indexed = df_cam.set_index('frame_number') # use the frame_number to be the index (the row labels)
    ...
    merged = cam_indexed.join(stim_subset, how='left') # making a df to store final data variables, with row index based on the laft table (cam_indexed)
